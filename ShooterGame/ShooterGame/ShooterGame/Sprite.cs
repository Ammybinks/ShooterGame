...
namespace SpriteLibrary
{
    ...
    public class Bullet : Unit
    {
        ...
        {
            Vector2 spawnLocation = GetSpawnLocation(owner);
            Vector2 origin = GetCenter();
            int speed = owner.firingSpeed;
            int directionAngle = owner.firingDirection;
            double rotationAngle = GetDirectionAngle() + 90;

            UpperLeft = spawnLocation;
            Origin = origin;
            SetSpeedAndDirection(speed, directionAngle);
        ...
        public Vector2 GetSpawnLocation(Sprite owner)
        {
            ...
            float X = (owner.UpperLeft.X + ownerCenter.X) - (GetWidth() / 2);
            float Y = (owner.UpperLeft.Y + ownerCenter.Y) - (GetHeight() / 2);
